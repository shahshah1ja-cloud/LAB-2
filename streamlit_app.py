...
with col_text:
    st.markdown("<h1 style='color: white;'>POLITEKNIK UNGKU OMAR</h1>", unsafe_allow_html=True)
    st.markdown("<h3 style='color: #00FF00;'>Jabatan Kejuruteraan Awam - Unit Geomatik</h3>", unsafe_allow_html=True)
...
def calculate_details(p1, p2, centroid_e, centroid_n):
    ...
    offset_e, offset_n = (norm_e / length) * 1.2, (norm_n / length) * 1.2
...
try:
    if file_ukur:
        ...
        fig.add_trace(go.Scatter(
            ...
            text=df['STN'], textposition="top right",
        ...
        # --- KONFIGURASI BACKGROUND SATELIT (ESRI) ---
        # Kami menggunakan gambar satelit sebagai background imej dalam koordinat Cartesian
        fig.update_layout(
            images=[dict(
                source="https://services.arcgisonline.com/arcgis/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}",
                xref="x", yref="y",
                x=df['E'].min() - 50, y=df['N'].max() + 50,
                sizex= (df['E'].max() - df['E'].min()) + 100,
                sizey= (df['N'].max() - df['N'].min()) + 100,
                sizing="stretch", opacity=0.7, layer="below"
            )],
            template="plotly_dark",
            # Sembunyikan Grid tetapi kekalkan Paksi (Easting/Northing)
            xaxis=dict(title="EASTING (m)", showgrid=False, zeroline=False, tickformat=".2f"),
            yaxis=dict(title="NORTHING (m)", showgrid=False, zeroline=False, tickformat=".2f", scaleanchor="x", scaleratio=1),
